...
class CenterFace(object):
    def __init__(self, landmarks=True):
        ...
        if self.landmarks:
            self.net = cv2.dnn.readNetFromONNX('/mnt/quedoulin/github_code/CenterNet/centerface.onnx')
        else:
...
#     else:
#         dets = decode(heatmap, scale, offset, None, (img_h_new, img_w_new), threshold=threshold)
#     if len(dets) > 0:
...
if __name__ == "__main__":
    frame = cv2.imread('/mnt/quedoulin/github_code/yolov5/data/images/bus.jpg')
    h, w = frame.shape[:2]
    landmarks = True
